...
`--nodes` is **mandatory, and must name exactly one node**. PVE treats it as
optional for every storage type; this plugin does not, because `storage.cfg`
is cluster-wide while the driver holds one array per node with the superblock
as a module parameter. An unrestricted storage would have every node in the
cluster contending for the same node-global array and the same lock, and two
named nodes cannot share it either. `check_config` refuses both on create,
and on update whenever `nodes` is being set.
...
The panel asks for the same options as `pvesm`. Leave Path empty and it
becomes `/mnt/pve/<id>`, and the remaining fields fall back to the defaults in
the table above. **ID and Nodes are both required** — Nodes must name the one
node that has the array, for the reason above, and the form enforces it.
